@php
    ...
    $getNavLinkClass = static function (string $routePrefix, array $item) use ($currentRoute): string {
        $itemRoute = $item['route'] ?? '';
        
        // Si no tiene ruta (padre collapsible), nunca marcar como active
        if (!is_string($itemRoute) || $itemRoute === '') return ' link-body-emphasis';

        // Comparar ruta exacta
        if ($currentRoute === $itemRoute) return ' active';
