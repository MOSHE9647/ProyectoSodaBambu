...
<tr class="detail-row" data-index="{{ $index }}">
        ...
        <select name="details[{{ $index }}][purchasable_id]" class="form-select form-select-sm purchasable-id" required>
            ...
            @else
                @foreach($supplies as $supply)
                    <option value="{{ $supply->id }}" data-price="0" {{ $purchasableId == $supply->id ? 'selected' : '' }}>
                        {{ $supply->name }}
                    </option>
                @endforeach
            @endif
    ...
    <td>
        <input type="number" name="details[{{ $index }}][unit_price]"
               ...
               value="{{ $detail->unit_price }}" min="0" step="0.01" required>
    </td>
    <td>
        <span class="subtotal">{{ number_format($detail->subtotal, 2) }}</span>
    </td>
    <td>
        <input type="date" name="details[{{ $index }}][expiration_date]"
               class="form-control form-control-sm"
               value="{{ $detail->expiration_date ? $detail->expiration_date->format('Y-m-d') : '' }}">
    </td>
    <td>
