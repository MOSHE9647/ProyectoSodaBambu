...
<div class="card-container rounded-2 p-4 w-75 justify-content-start">
    <form id="{{ isset($supplier) ? 'edit-supplier-form' : 'create-supplier-form' }}" action="{{ $action }}" method="POST" class="d-flex flex-column gap-2">
        {{-- CSRF Token --}}
        @csrf
        @if(isset($supplier))
        @method('PUT')
        ...
        <section id="basic-information" class="d-flex flex-column mb-4 gap-3">
            ...
            <div class="row g-3">
                {{-- Name --}}
                <div class="col-12">
                    <x-form.input :id="'name'" :type="'text'" :class="'border-secondary'" :inputClass="$errors->has('name') ? 'is-invalid' : ''" :placeholder="'Ej: Distribuidora Central S.A.'" :value="old('name', optional($supplier)->name ?? '')" :errorMessage="$errors->first('name') ?? ''" :iconLeft="'bi bi-building'" :required="true">
            ...
            <div class="row g-3">
                {{-- Phone --}}
                <div class="col-md-6">
                    <x-form.input :id="'phone'" :type="'tel'" :class="'border-secondary'" :inputClass="$errors->has('phone') ? 'is-invalid' : ''" :errorMessage="$errors->first('phone') ?? ''" :placeholder="'+506 XXXX XXXX'" :value="old('phone', optional($supplier)->phone ?? '')" :iconLeft="'bi bi-telephone'" :required="true">
                ...
                {{-- Email --}}
                <div class="col-md-6">
                    <x-form.input :id="'email'" :type="'email'" :class="'border-secondary'" :inputClass="$errors->has('email') ? 'is-invalid' : ''" :errorMessage="$errors->first('email') ?? ''" :placeholder="'proveedor@ejemplo.com'" :value="old('email', optional($supplier)->email ?? '')" :iconLeft="'bi bi-envelope'" :required="true">
