...
<x-header :title="$isEdit ? 'Editar Insumo' : 'Crear Insumo'" :subtitle="$isEdit ? 'Actualice los datos del insumo' : 'Registra un nuevo insumo para el inventario'" />

<div class="table-container rounded-2 p-4 w-75 justify-content-start">
    <form id="{{ $isEdit ? 'edit-supply-form' : 'create-supply-form' }}" action="{{ $action }}" method="POST" class="d-flex flex-column gap-2">
        @csrf
        ...
        <section id="basic-information" class="d-flex flex-column mb-4 gap-3">
            ...
            <div class="row g-3">
                <div class="col-6">
                    <x-form.input :id="'name'" :type="'text'" :class="'border-secondary'" :inputClass="$errors->has('name') ? 'is-invalid' : ''" :placeholder="'Ej: Harina de Trigo'" :value="old('name', $supply->name ?? '')" :errorMessage="$errors->first('name') ?? ''" :iconLeft="'bi bi-tag'" :required="true">
                ...
                <div class="col-6">
                    <x-form.input :id="'quantity'" :type="'number'" :class="'border-secondary'" :inputClass="$errors->has('quantity') ? 'is-invalid' : ''" :placeholder="'0'" :value="old('quantity', $supply->quantity ?? '0')" :errorMessage="$errors->first('quantity') ?? ''" :iconLeft="'bi bi-hash'" :required="true">
                ...
                <div class="col-6">
                    <x-form.input :id="'expiration_date'" :type="'date'" :class="'border-secondary'" :inputClass="$errors->has('expiration_date') ? 'is-invalid' : ''" :value="old('expiration_date', (isset($supply->expiration_date) && $supply->expiration_date) ? \Carbon\Carbon::parse($supply->expiration_date)->format('Y-m-d') : '')" :errorMessage="$errors->first('expiration_date') ?? ''" :iconLeft="'bi bi-calendar-event'">
                        Fecha de Vencimiento
                    </x-form.input>
                </div>

                <div class="col-6">
                    <x-form.input :id="'expiration_alert_days'" :type="'number'" :class="'border-secondary'" :inputClass="$errors->has('expiration_alert_days') ? 'is-invalid' : ''" :placeholder="'7'" :value="old('expiration_alert_days', $supply->expiration_alert_days ?? '7')" :errorMessage="$errors->first('expiration_alert_days') ?? ''" :iconLeft="'bi bi-bell'">
