...
const ClosureModalOptions = {
    popup: 'swal-popup-dark bg-dark text-white rounded-4 border-success shadow-lg', 
...
const showClosureModal = async (registerId) => {
    try {
        ...
        SwalModal.fire({
            ...
            customClass: ClosureModalOptions,
            width: '550px',
            html: `
                <div class="text-start p-2">
                    <div class="mb-4 d-flex justify-content-between align-items-center border-bottom border-secondary pb-2">
                        ...
                        <strong class="fs-5">Total Sistema: ₡ ${formatCR(data.system_total)}</strong>
                    </div>
                    
                    <p class="small text-muted mb-3 text-uppercase fw-bold ls-wide">Validación de Medios de Pago</p>

                    <!-- EFECTIVO -->
                    <div class="bg-secondary bg-opacity-10 p-3 rounded-3 mb-2 border border-secondary shadow-sm">
                        <div class="d-flex justify-content-between mb-2 align-items-center">
                            <span><i class="bi bi-cash-stack me-2 text-success"></i>Efectivo</span>
                            <span class="text-muted small">Sistema: ₡ ${formatCR(data.system_cash)}</span>
                        </div>
                        <div class="input-group input-group-sm">
                            <span class="input-group-text bg-dark text-white border-secondary">₡</span>
                            <input type="number" id="physical-cash" class="form-control bg-dark text-white border-secondary" placeholder="Monto en efectivo">
                        </div>
                        <div id="cash-feedback" class="small mt-2 d-none p-2 rounded"></div>
                    </div>

                    <!-- TARJETA -->
                    <div class="bg-secondary bg-opacity-10 p-3 rounded-3 mb-2 border border-secondary border-opacity-50">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <span><i class="bi bi-credit-card me-2 text-primary"></i>Tarjeta</span>
                            <span class="text-muted small">Sistema: ₡ ${formatCR(data.system_card)}</span>
                        </div>
                        <div class="input-group input-group-sm">
                            <span class="input-group-text bg-dark text-white border-secondary">₡</span>
                            <input type="number" id="physical-card" class="form-control bg-dark text-white border-secondary" placeholder="Monto según váuchers">
                        </div>
                        <div id="card-feedback" class="small mt-2 d-none p-2 rounded"></div>
                    </div>

                    <!-- SINPE MÓVIL -->
                    <div class="bg-secondary bg-opacity-10 p-3 rounded-3 mb-3 border border-secondary border-opacity-50">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <span><i class="bi bi-phone me-2 text-info"></i>SINPE Móvil</span>
                            <span class="text-muted small">Sistema: ₡ ${formatCR(data.system_sinpe)}</span>
                        </div>
                        <div class="input-group input-group-sm">
                            <span class="input-group-text bg-dark text-white border-secondary">₡</span>
                            <input type="number" id="physical-sinpe" class="form-control bg-dark text-white border-secondary" value="${formatCR(data.system_sinpe)}" placeholder="Monto según transferencias">
                        </div>
                        <div id="sinpe-feedback" class="small mt-2 d-none p-2 rounded"></div>
                    </div>

                    <div class="mt-3">
                        <label class="small text-muted mb-1 text-uppercase fw-bold">Notas de Cierre (Opcional)</label>
                        <textarea id="closure-notes" class="form-control bg-dark text-white border-secondary" rows="2" placeholder="Observaciones adicionales..."></textarea>
                    </div>
