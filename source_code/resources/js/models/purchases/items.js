import { enableBootstrapTooltips, formatCurrency } from "../../utils/utils.js";
...
const calculateTotals = () => {
	...
	rows.each((_, row) => {
		...
		if (!isNaN(quantity) && !isNaN(unitPrice) && subTotalSpan.length) {
			...
			subTotalSpan.text(formatCurrency(subTotal, false));
			total += subTotal;
			itemCount += 1;
		}
	});

	elements.tableFootTotal.text(formatCurrency(total, false));
        ...
        <tr data-id="" data-purchasable-type="${modelClass}">
            ...
            <td>
                <div class="item-quantity d-flex flex-row align-items-center justify-content-center gap-2">
                    ...
                    <input name="quantity" type="number" class="quantity-input form-control border-0 text-center fw-semibold text-body px-1 py-0" style="width: 38px; background-color: transparent;" value="1" min="1" step="1" required>
