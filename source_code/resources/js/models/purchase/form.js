...
$('#add-detail').on('click', function () {
    ...
    const template = `
        <tr class="detail-row" data-index="${index}">
            ...
            <td>
                <input type="number" name="details[${index}][subtotal]" class="form-control form-control-sm subtotal-input" value="0" min="0" step="0.01" required>
            </td>
            <td>
...
function submitPurchaseForm() {
    ...
    if (!$total.val() || parseFloat($total.val()) < 0) {
        SwalToast.fire({ icon: 'error', text: 'El total de la compra es obligatorio.' });
        return false;
    }
    ...
    $('#details-container .detail-row').each(function () {
        ...
        const subtotal      = $(this).find('.subtotal-input').val();
        ...
        if (subtotal === '' || parseFloat(subtotal) < 0) {
            $(this).find('.subtotal-input').addClass('is-invalid');
            valid = false;
        } else {
            $(this).find('.subtotal-input').removeClass('is-invalid');
        }
    });
...
$('#quick-product-form').on('submit', async function (e) {
    ...
    try {
        ...
        // EIF-170: Nunca enviar stock_actual
