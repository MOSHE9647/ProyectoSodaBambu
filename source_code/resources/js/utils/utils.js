...
export function setLoadingState(formId, isLoading) {
	...
	if (!submitButton.length || !spinner.length || !submitButtonText.length) {
		console.error(
			`Submit button or spinner not found for form ID: ${formId}`,
		);
		return;
	}
...
export function toggleLoadingState(element, elementClass, isLoading) {
	...
	spinner
		...
		?.classList.toggle("visually-hidden", !isLoading);
...
export function togglePasswordVisibility(inputId, toggleButtonId) {
	...
	if (!input || !toggleButton) {
		console.error(
			"Input or toggle button not found:",
			inputId,
			toggleButtonId,
		);
		return;
...
export const calculateAlertDate = () => {
	...
	if (dateString) {
		...
		const alertDateFormatted = expiration.toLocaleDateString(
			"es-ES",
			options,
		);
